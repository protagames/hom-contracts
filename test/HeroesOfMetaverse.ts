...
// describe.only("HOM", function () {
describe("HOM", function () {
  const DEAD_ADDRESS = "0x000000000000000000000000000000000000dEaD";
  const BUSD_ADDRESS = "0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56";
  const ROUTER_ADDRESS = "0x10ED43C718714eb63d5aA57B78B54704E256024E";
  let WETH: string;
  let contract: HOMToken;
  let router: IUniswapV2Router02;
  let factory: Contract;
  let busdContract: Contract;
  const busdHotWalletAddress: string =
    "0x8894e0a0c962cb723c1976a4421c95949be2d4e3"; // to impersonificate a wallet with lots of busd when local forking the mainet
  ...
  beforeEach(async function () {
    const HomFactory = <HOMToken__factory>(
      await ethers.getContractFactory("HOMToken")
    );
    contract = await HomFactory.deploy();
  ...
  it("Should be constructed properly", async function () {
    expect(await contract.name()).to.equal("Heroes of Metaverse Token");
    expect(await contract.symbol()).to.equal("HOM");
    expect(await contract.totalSupply()).to.equal(
      expandTo18Decimals(420000000)
    );
